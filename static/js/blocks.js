...
class Document {
    ...
    addPara(para, idx=null, after=null, before=null) {
        if (idx === null) {
            if (after !== null) {
                idx = this.paraOrder.indexOf(after) + 1;
            } else if (before !== null) {
                idx = this.parakOrder.indexOf(before);
            } else {
                idx = this.paraOrder.length;
            }
        }
        this.paras[para.pid] = para;
        this.paraOrder.splice(idx, 0, para.pid);
    }

    // remove a paragraph from the document
    removePara(para, pop=false) {
        let idx = this.paraOrder.indexOf(para.pid);
        if (pop) {
            return this.paraOrder.splice(idx, 1)[0];
        } else {
            delete this.paras[para.pid];
        }
    }
